callNCW <- function(title,label,nperm = 10, ncore=1,seedn=100,stability=TRUE) {
  ...
  #' @param ncore A integer value of cores to use, or ncore=1 (default). It's the input core numbers for the parallel computation in R package \code{parallel}.
  ...
  randL <- parallel::mclapply(seedL,function(x){randConsensusMatrix(l.seed=x,l.label=label,l.ns=ns,l.nc=nc,l.nv=nv,l.index=index,l.pair.ind=pair.ind,l.ppath=ppath)},mc.cores=ncore)
  ...
  if (stability==TRUE) {
    # calculate normalized censensus weight for every step(1000)
    ...
    ### prepare pdata for plot, output to paste0(title,".seed",seedn,".n",nperm,"K.pdf")
    ### squared euclidean distance for each cell
    ...
    ### overall squared euclidean distance
    ...
    ggplot2::ggsave(paste0(title,"stability.seed",seedn,".n",nperm,"K.pdf"),width=max(8,nperm*0.5),height=5)
    message(paste("Finished estimation of stability of permutation numbers for normalized consensus weight:\n"))

  } else {
    # calculate normalized censensus weight for all permutated results
